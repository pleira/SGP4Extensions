...
class NearCheck extends FunSuite  {
...
//  implicit val doubleEquality = TolerantNumerics.tolerantDoubleEquality(8E-2)
  implicit val doubleEquality = TolerantNumerics.tolerantDoubleEquality(1E-3)

    test("structure all") ({
    assert(tles.size == 3)

    val pfs  = tles map (tle => Sgp4Result(tle) _ )  // partially applied function
    
    val tle5 = tles(0)     ;  val pf5      = pfs(0)
    val tle6251 = tles(1)  ;  val pf6251   = pfs(1)
    val tle28057 = tles(2) ;  val pf28057  = pfs(2)
    ...
    val results5 = for (t <- times5) yield pf5(t)
    ...
    val results6251 = for (t <- times6251) yield pf6251(t)
    ...
    val results28057 = for (t <- times28057) yield pf28057(t)
    
    // call the check for the corresponding result
