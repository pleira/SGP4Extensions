...
trait SPNLongPeriodCorrections[F] extends SimpleKeplerEq {
  ...
  def lppCorrections(pn: (SpecialPolarNodal[F], AuxVariables[F]), secElemt: SGPElems[F])(implicit ev: Field[F], nr: NRoot[F], trig: Trig[F])
       : (SpecialPolarNodal[F], LongPeriodContext[F]) = {
    import pn._1._,pn._2.{p,s,c},wgs.{KE,`J3/J2`,twopi}, secElemt.n
