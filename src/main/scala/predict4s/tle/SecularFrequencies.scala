...
/**
 * The initialization provides the initial mean elements at epoch.
 * From them, the Delaunay elements at epoch can be obtained.
 * From these Delaunay elements, Brouwer’s theory provides the secular frequencies 
 * due to the earth’s gravitational potential.
 * These frequencies remain constant for the evaluation of the theory at different dates.
 */
case class SecularFrequencies[F: Field: NRoot : Order: Trig](elem: TEME.SGPElems[F], ctx0: Context0[F], gcof : GeoPotentialCoefs[F], gctx : GeoPotentialContext[F]) {
