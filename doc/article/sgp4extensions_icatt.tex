...
The SGP4 algorithms are implemented in SGP4Extensions with a intentional
separation of the run time part, like reading/writing from/to files, called
...
As example, the code for the Lyddane 2nd Order Long Period Corrections is:
%\begin{Scala}
\begin{minted}{scala}
val δI =  ϵ3*esinω* c
val δa =  0
...
Figures (\ref{fig:dx})--(\ref{fig:dr}) show the comparison of propagation results for the test TLE 00005 for 1 day with Vallado's C++ results for the different algorithms.
